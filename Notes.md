...
[x] Section 3.1 - Add a bit more detail (if word count allows)
...
- Expand language to include many more rules and brute force limiting max size
	...
	- Test the work flow of finding best strategies
...
- Implement a progressive brute force, gradually increase number of runs, saving a data file for every number of runs, only run strategies remaining based of Mann-Whitney
...
3. Decide on a perturbation method
	- If wanting to do iterated local search need to find a way to try and jump out of the local maxima
		...
		- vary the composition of the try and ban moves?
