use super::{
    local::local_search_try_restart, print_best_strategy_info, strategy_duel, Runs, SnakeData,
    StrategyDuelResult,
};
...
fn iterated_local_search(
    ...
    println!("\n\nStarting ILS...");
    let mut global_best = local_search_try_restart(engine, snake_data);
    ...
    for count in 0..max_count {
        ...
        println!("\nMutated:\n{}", mutated.strategy);
        let mut mutated_best = local_search_try_restart(engine, mutated);
        match variation {
            ILSVariation::AlwaysAccept => {
